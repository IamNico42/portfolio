...
|Schritt|Erklärung|
|---|---|
|1️⃣|TCP empfängt **3 DupACKs** für z. B. ACK 3|
|2️⃣|**Fast Retransmit**: TCP **sendet verlorenes Segment** sofort neu (z. B. S3)|
|3️⃣|**Fast Recovery beginnt**:|
|→ `ssthresh = cwnd / 2`||
|→ `cwnd = ssthresh + 3` _(wegen der 3 DupACKs)_||
|4️⃣|Für jedes weitere DupACK → `cwnd += 1` _(um den Durchsatz zu halten)_|
|5️⃣|Wenn ein echtes ACK kommt (nicht dup), das die Lücke schließt →|
|→ `cwnd = ssthresh`||
|→ TCP geht in **Congestion Avoidance**, **nicht** zurück in Slow Start||
